...
## Rules
- Use [Conventional Commits](https://www.conventionalcommits.org/): `<type>(<scope>)?: <subject>`. Types in use here: `feat`, `fix`, `chore`, `docs`, `refactor`, `test`, `build`, `ci`. The `chore(ai):` scope is reserved for cc-project-director's plan-bookkeeping commits (see the cc-project-director system prompt).
...
- cc-project-director owns all git commits on this repo; other subagents dispatched by it do not commit.
