...
Before finalising the Phase 6 proposal, consult skillex to see whether any pre-built skills already cover what the audit and Discovery surfaced — this is why the plugin ships the `skillex` MCP server, and skipping it defeats its purpose:

- Call `mcp__skillex__search_skills` with **2–4 targeted queries** derived from **both the audit findings and the Discovery answers** (good query sources: audit-detected language/runtime, testing approach, deployment target, domain keywords from the audit and Discovery). Keep queries tight and distinct — this is a sampling pass, not exhaustive enumeration. Over-calling wastes tokens; under-calling defeats the purpose.
